...
- **Privacy First**: All data is stored locally on your device using the browser's storage API. No data is ever sent to external servers. Zero network requests — no analytics, no CDNs, nothing.
- **Zero Footprint**: Scripts are only injected into pages when you actually use the extension — no always-on content scripts running in every tab.
- **Quick Add Workflow**: Highlight text on any webpage, right-click, and select "Add to Smart Paster" to save it instantly via an inline overlay — no navigating away.
- **Backup & Restore**: Export your entire snippet collection to JSON and import it on other devices or browsers.
- **Comfortable Grid Dashboard**: A responsive, modern terminal-inspired dashboard for managing your collection.
...
2.  **To Add**: Use the Quick Input bar at the top of the dashboard.
...
4.  Enter a label for the snippet in the overlay and click **Save**.
